...
class NVFP4SVDLinearMethod(NVFP4LinearMethod):
    ...
    # Checkpoint tensors consumed by this method (loaded here, not module
    # parameters), keyed by suffix. Models exclude them from strict
    # model-vs-checkpoint key validation.
    PROVIDED_CKPT_SUFFIXES = (
        ".pre_quant_scale",
        ".svdquant_lora_a",
        ".svdquant_lora_b",
    )
    ...
    def load_weights_vanilla(self,
        ...
        if getattr(module, "pre_quant_scale",
                   None) is None and "pre_quant_scale" in w:
    ...
    def transform_weights(self, module) -> None:
        ...
        # CUDA graph capture.
        l2t_smoothed = (pqs.to(torch.bfloat16).unsqueeze(1) *
                        lora_a.t().to(torch.bfloat16)).contiguous()
        l1_scaled = ((lora_b.to(torch.float32) /
                      module.alpha.reshape(-1)[:1]).to(
    ...
    def apply(self, module, input, bias):
        ...
        if a is not None and b is not None and pqs is not None:
            ...
            return self._apply_svdquant_reference(module, input, bias, pqs, a,
                                                  b)
    ...
    def _quantize_svdquant_input(module, x2d, pqs):
        global_scale = module.input_scale.to(
            torch.float32).reshape(1).contiguous()
        return torch.ops.trtllm.nvfp4_quantize_smooth(
            x2d,
            pqs.to(torch.bfloat16).contiguous(), global_scale)
